...
const AdminPanel = ({ personas, onSave, onClose }) => {
  ...
  const addPersona = () => {
    ...
    setEdited(prev => ({ ...prev, [key]: { name: "Nouveau Persona", age: "— ans — Ville", job: "Profession · Diplôme · CSP", emoji: "🎯", type: t.value, typeLabel: t.label, color: t.color, medias: [], interests: [], frustrations: [], hooks: [], triggers: [], bio: "Biographie du persona.", lastUpdate: null, signals: [] } }));
  ...
  const fi = { label: { fontSize: 11, letterSpacing: 1, textTransform: "uppercase", color: "#8a8070", marginBottom: 4, display: "block" }, input: { width: "100%", border: "1px solid #e0d8cc", borderRadius: 3, padding: "7px 10px", fontFamily: "inherit", fontSize: 13, outline: "none", background: "white", color: "#0d0d0d" } };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "white", borderRadius: 8, width: "100%", maxWidth: 860, maxHeight: "90vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          <div style={{ width: 200, borderRight: "1px solid #e0d8cc", background: "#fafaf8", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: 12, borderBottom: "1px solid #e0d8cc" }}>
              ...
              {Object.entries(edited).map(([key, pe]) => (
                <div key={key} onClick={() => setActiveKey(key)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 4, cursor: "pointer", marginBottom: 2, background: activeKey === key ? "#0d0d0d" : "transparent", color: activeKey === key ? "white" : "#0d0d0d" }}>
                  <span>{pe.emoji}</span>
                  <span style={{ fontSize: 13, fontWeight: 500, flex: 1 }}>{pe.name}</span>
            ...
            <div style={{ padding: 12 }}>
              {newPersonaMode ? (
                <div>
                  <input value={newKey} onChange={e => setNewKey(e.target.value)} placeholder="Nom clé (ex: nouveau)" style={{ ...fi.input, marginBottom: 6, fontSize: 12 }} autoFocus />
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={addPersona} style={{ flex: 1, background: "#1a6b3a", color: "white", border: "none", borderRadius: 3, padding: "6px", fontSize: 11, cursor: "pointer" }}>✓ Créer</button>
          ...
          <div style={{ flex: 1, overflow: "auto", padding: 24 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
              ...
              <div>
                <label style={fi.label}>Emoji</label>
            ...
            <div style={{ marginBottom: 16 }}><label style={fi.label}>Profession & profil</label><input style={fi.input} value={p.job} onChange={e => update("job", e.target.value)} /></div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {[["medias","Médias & Plateformes"],["interests","Centres d'intérêt"],["hooks","Points d'accroche"],["triggers","Déclencheurs"],["frustrations","Frustrations"]].map(([key, label]) => (
                <div key={key} style={{ background: "#fafaf8", border: "1px solid #e0d8cc", borderRadius: 4, padding: 12 }}>
                  <label style={fi.label}>{label}</label>
...
export default function PersonaLab() {
  ...
  const analyze = async () => {
    ...
    try {
      const sys = `Tu es ${p.name}, ${p.age}. Tu fais partie des "${p.typeLabel}". Bio: ${p.bio} Médias: ${p.medias.join(", ")}. Intérêts: ${p.interests.join(", ")}. Accroches: ${p.hooks.join(", ")}. Frustrations: ${p.frustrations.join(", ")}. Réponds UNIQUEMENT en JSON valide sans backticks ni markdown: {"score":<0-100>,"verdict":"<max 8 mots>","reaction":"<1ère personne 2-3 phrases>","dimensions":{"pertinence":<0-100>,"format":<0-100>,"ton":<0-100>,"engagement":<0-100>},"recommandations":["<1>","<2>","<3>"]}`;
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px 60px" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 22, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {Object.entries(personas).map(([key, pe]) => (
            <button key={key} onClick={() => selectPersona(key)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "7px 14px", border: `1.5px solid ${current === key ? "#0d0d0d" : "#e0d8cc"}`, background: current === key ? "#0d0d0d" : "white", color: current === key ? "white" : "#0d0d0d", borderRadius: 40, cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>
              <span>{pe.emoji}</span>{pe.name} — {pe.typeLabel}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 20, alignItems: "start" }}>
          <div style={{ background: "#0d0d0d", color: "white", borderRadius: 4, overflow: "hidden", position: "sticky", top: 20 }}>
            ...
            <div style={{ padding: "14px 18px" }}>
              {[{ t: "Biographie", c: p.bio },{ t: "Médias", tags: p.medias },{ t: "Accroches", tags: p.hooks },{ t: "Déclencheurs", c: p.triggers.join(" · ") },{ t: "Frustrations", c: p.frustrations.join(" · "), red: true }].map((item, i) => (
                <div key={i} style={{ marginBottom: 12 }}>
                  ...
                  {item.tags ? <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{item.tags.map((t, j) => <span key={j} style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 3, padding: "1px 6px", fontSize: 11, color: "rgba(255,255,255,0.65)" }}>{t}</span>)}</div>
            ...
            {updatePanel && (
              <div style={{ background: "white", border: "1.5px solid #c9a84c", borderRadius: 4, overflow: "hidden" }}>
                <div style={{ background: "#fffbf0", padding: "14px 18px", borderBottom: "1px solid #e8e0c8", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div><div style={{ fontSize: 13, fontWeight: 600 }}>↻ Signaux détectés pour {p.name}</div><div style={{ fontSize: 11, color: "#8a8070", marginTop: 2 }}>3 évolutions proposées — validez avant application</div></div>
